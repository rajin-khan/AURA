...
class DisplacementFeatures:
    """Feature bundle for a (original, edited) pair."""
...
def compute_displacement_features(e_orig: np.ndarray, e_edit: np.ndarray) -> DisplacementFeatures:
    """Compute displacement vector + small, robust scalar features.

    We keep this minimal on purpose:
    - d = E(edit) - E(orig)
    - norms + a few cosine similarities are surprisingly strong baselines.

    Future work:
    - subspace projection features (PCA) and residual norms
    - domain-conditioned direction families
    """
...
def vectorize(d: DisplacementFeatures, *, include_raw: bool = False) -> Tuple[np.ndarray, Dict[str, int]]:
    ...
    If include_raw=True, we append the displacement vector itself.
    (This can work well for linear probes, but increases dimensionality.)
    """
